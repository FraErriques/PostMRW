...
void Process_MonteCarlo_caller()
{
...
void massive_UnitTest_caller_Cantiere()
{// NB amplitude==0
    //    //------Unit Test-----CANTIERE------------------------------------------------
    ...
    //
    Test_Unit_CantierePrimes * test = new Test_Unit_CantierePrimes( 0);// NB amplitude==0  ####------########
    bool seq = test->sequentialDump( 99390);// required prime==soglia
    seq &= true;

    bool rand = test->randomDump(  15,  20);
    rand &= true;
    rand = test->randomDump( 900, 920);// i.e. dump Primes on custom interval
...
void massive_UnitTest_caller_Produzione()
{// NB amplitude==80
    ////    ------Unit Test-----PimesFinder--------test_PRODUCTION_----------------------------------------
    Test_Unit_PrimesFinder * test_PRODUCTION_ = new Test_Unit_PrimesFinder( 80);// NB amplitude
    ...
    {
        p_reader &= test_PRODUCTION_->readBy_OperatorSquares( c);// ask Prime[n]
    }
    std::cout<<"\n\n\n\t the final outcome is : "<< p_reader<<"\n\n";
    std::cout<<"\n\n\n\t Strike Enter to continue\t";
    getchar();
    p_reader &= test_PRODUCTION_->readSequentialDump_nextRec( 60);
    p_reader &= test_PRODUCTION_->readSequentialDump_arrayOfRec_anywhere(
    23
    ,915 // there must be room for just one record Prime<100.
    );
    for( int c=0; c<61; c++)
    {// next Rec
        p_reader &= test_PRODUCTION_->readSequentialDump_nextRec(c);
    }
    for( int c=0; c<61; c++)
    {// array of Rec
        p_reader &= test_PRODUCTION_->readSequentialDump_arrayOfRec_anywhere(
...
}//massive_UnitTest_caller_Produzione
...
//    std::cout<<"\n\n\t "<< c_buffer<<"\n\n";
    ...
    Process::testCpp11_MonteCarlo(); //  undefined reference on Win
